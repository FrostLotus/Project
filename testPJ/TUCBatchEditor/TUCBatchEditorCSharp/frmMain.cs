...
namespace TUCBatchEditorCSharp
{
    ...
    {
        /// <summary>
        /// [STRUCT]RECT 結構會依其左上角和右下角的座標來定義矩形
        /// </summary>
        private struct RECT
        {
            //原始是long
        ...
        /// <summary>
        /// [ENUM]零起始位移,額外視窗記憶體的位元組數目
        /// </summary>
        public enum GETWINDOW_LONGCODE : int
        {
            /// <summary>
            /// 擷取 擴充視窗樣式。
            /// </summary>
        ...
        /// <summary>
        /// [STATIC CLASS]所指定視窗與要擷取其控制碼的視窗之間的關聯性。 此參數為下列其中一個值。
        /// </summary>
        ...
        private class HandleObject
        {
            ...
            public HandleObject(HANDLE_TYPE e, Form xCtrl)
            {
                eType = e;
                this.xCtrl = xCtrl;
            }
            public HANDLE_TYPE eType;
            public Form xCtrl;
        }
        //====================================================================
        ...
        [DllImport("user32.dll")]
        static extern int GetSystemMetrics(SYSTEM_METRICCODE smIndex);
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            m_lsHandle.Add(new HandleObject(HANDLE_TYPE.Main, this));//控制項註冊
            #endregion
            ...
            int nLeft = GetSystemMetrics(SYSTEM_METRICCODE.SM_XVIRTUALSCREEN);//左座標=0
            int nTop = GetSystemMetrics(SYSTEM_METRICCODE.SM_YVIRTUALSCREEN);//頂座標=0
            int nRight = GetSystemMetrics(SYSTEM_METRICCODE.SM_CXVIRTUALSCREEN);//右座標
            int nBottom = GetSystemMetrics(SYSTEM_METRICCODE.SM_CYVIRTUALSCREEN);//底座標
            ...
            AOI_CUSTOMERTYPE eCustomerType = AOI_CUSTOMERTYPE.CUSTOMER_TTA_TEST;//TTA mode (TEST)
            if (args.Length >= 2)//有回傳
            {
                if (int.TryParse(args[1], out int nTemp))//可以換成int
                {
                    eCustomerType = (AOI_CUSTOMERTYPE)nTemp;
                }
            }
            for (int i = 0; i < nUnit; i++)
            {
                ...
                //for top most
                m_lsHandle.Add(new HandleObject(HANDLE_TYPE.Form, xFormTmp));//處裡視窗加入控制項
        ...
        private void TopMostTimer_Tick(object sender, EventArgs e)
        {
            ...
            //RECT rc = new RECT();
            long nWndStyle = GetWindowLong((IntPtr)nWinHandle, (int)GETWINDOW_LONGCODE.GWL_STYLE);//Form訊息
        ...
        private void TopmostEditForm(Form xForm)
        {
            ...
            m_lsHandle.Where(x => {
                ...
                return x.eType == HANDLE_TYPE.EditForm && rcForm.IntersectsWith(rcEdit);
        ...
        {
            m_lsHandle.Add(new HandleObject(HANDLE_TYPE.EditForm, xAdd));
        }
        /// <summary>
        /// 移除控制項
        /// </summary>
