...
void CSystDONGGUANPPProcess::IninOPCClient()
{
	...
	if (m_ppFIELD_INFO)//訂閱存在
	{
		for (int i = 0; i < nFieldSize; i++)
		{
			if (m_ppFIELD_INFO[i])//項目存在
			{
				//將節點名稱(英數)加在vNodeName最後面 將所有節點名稱串到vector起來
...
void CSystDONGGUANPPProcess::Init()
{
	...
	m_ppFIELD_INFO = new SubscribeNodeItem * [FIELD_MAX];//全範圍的訂閱監控項目
	for (int i = 0; i < FIELD_MAX; i++)
	{
		m_ppFIELD_INFO[i] = new SubscribeNodeItem;
		memset(m_ppFIELD_INFO[i], 0, sizeof(SubscribeNodeItem));//大概偏向 *m_ppFIELD_INFO[i]=0;
		///memcpy(m_ppFIELD_INFO[i], &ctSYST_FIELD[i], sizeof(NodeItem));
		m_ppFIELD_INFO[i]->nFieldId = ctSYST_FIELD[i].nFieldId;//(int)nFieldId
		memcpy(m_ppFIELD_INFO[i]->strName, ctSYST_FIELD[i].strName, sizeof(ctSYST_FIELD[i].strName));//(TCHAR)strName
		memcpy(m_ppFIELD_INFO[i]->strNodeId, ctSYST_FIELD[i].strNodeId, sizeof(ctSYST_FIELD[i].strNodeId));//(TCHAR)strNodeId 
		m_ppFIELD_INFO[i]->nLen = ctSYST_FIELD[i].nLen;//(int)nLen
		m_ppFIELD_INFO[i]->nType = ctSYST_FIELD[i].nType;//(int)nType
		m_ppFIELD_INFO[i]->xTime = ctSYST_FIELD[i].xTime;//(time_t)xTime
		m_ppFIELD_INFO[i]->pValue = new BYTE[ctSYST_FIELD[i].nLen];//*pValue先定義
		memset(m_ppFIELD_INFO[i]->pValue, 0, ctSYST_FIELD[i].nLen);//(BYTE)pValue
#ifdef _DEBUG
...
void CSystDONGGUANPPProcess::ON_RECEIVE_AOIDATA(OPCDataType eType)
{
	switch (eType)
	{
	case OPCDataType::INSP://瑕疵資料
	{
		OPCInspData xData;
		memset(&xData, 0, sizeof(OPCInspData));//初始化
		if (USM_ReadAOIData((BYTE*)&xData, sizeof(OPCInspData)))//讀AOIDATA成功
		{
#ifdef WRITE_OPCFIELD_THREAD
...
#else
			m_vInspData.push_back(xData);//將讀到資訊串在瑕疵資料vector最後面
			ProcessInspData(); //處理瑕疵資料
#endif
		}
	}
	break;
	case OPCDataType::BATCH://工單資料
	{
		OPCBatchData xData;
		memset(&xData, 0, sizeof(OPCBatchData));//初始化
		if (USM_ReadAOIData((BYTE*)&xData, sizeof(OPCBatchData)))//讀AOIDATA成功
		{
#ifdef WRITE_OPCFIELD_THREAD
...
#else
			m_vBatchData.push_back(xData); //將讀到資訊串在瑕疵資料vector最後面
			ProcessNewBatchDone();//處理工單資料
#endif
		}
	}
	break;
	}
	NotifyAOI(WM_OPC_FINISHWRITE_CMD, eType);
}
...
BOOL CSystDONGGUANPPProcess::GetUANodeId(int nField, UA_NodeId& xNodeId)
{
	...
	if (m_ppFIELD_INFO)//若存在
	{
		...
		/*
		if(nField >= 0 && nField < nFieldSize)//不跑迴圈
		{
			if (m_ppFIELD_INFO[nField])
			{
				xNodeId = m_ppFIELD_INFO[nField]->xNodeId;
				return TRUE;
			}
		}
		*/
...
#ifdef WRITE_OPCFIELD_THREAD
						if (pItem->nFieldId == FIELD_Z_QXFLAG) { // mark flag in thread. prevent opc error
							::SetEvent(m_hEvent[bData ? EV_INSP_TRUE : EV_INSP_FALSE]);
						}
						else if (pItem->nFieldId == FIELD_P_FLAG) {// mark flag in thread. prevent opc error
...
void CSystDONGGUANPPProcess::ProcessNewBatch()
{
	...
	xData.MENGE = *(float*)m_ppFIELD_INFO[FIELD_MENGE]->pValue;
	...
	{
		NotifyAOI(WM_OPC_NEWBATCH_CMD, 1/*0:九江;1:松八*/);//通知AOI視窗新工單資訊
	}
	else
	{
...
void RemoveOldestFileIfOverLimit(const CString& folderPath, int limit)
{
	...
	while (bWorking) {
		...
		if (!finder.IsDots() && !finder.IsDirectory())
		{
			...
			BOOL bSkip = FALSE;
			for (auto& xReserveFile : ctYJDFiles)//保留檔案不移除
			{
...
#ifdef _DEBUG
	RemoveOldestFileIfOverLimit(strFolder, 1); //only keep latest 500 files
#else
...
#define INI_R3DATA L"_R3DATA.txt"
	...
	strFilePath.Format(L"%s\\%s",
		strFolder, //路徑
		INI_R3DATA);//"_R3DATA.txt"
//寫入
	FILE* stream = _wfsopen(strFilePath, L"ab", _SH_DENYRW);
	if (stream)
	{
		///寫在最後面  感覺後面要時間
		//订单号,批次號,物料,数量(批次米数/张数),单位 = 123,AAA,456,111.20,789
		fwprintf(stream, L"%s,%s,%s,%.2f,%s\r\n",
			CString(xData.AUFNR, _countof(xData.AUFNR)),//订单号
			CString(xData.CHARG, _countof(xData.CHARG)),//批次號
			CString(xData.MATNR, _countof(xData.MATNR)),//物料
			xData.MENGE,//数量(批次米数/张数)
			CString(xData.MEINS, _countof(xData.MEINS))//单位
		);
		fclose(stream);
	}
}
...
void CSystDONGGUANPPProcess::ProcessInspData()
{
	...
	if (!bFlag)
	{
		...
		EnterCriticalSection(&m_xLock);
#endif
		int nCmdSize = m_vInspData.size();//若瑕疵資料有資料寫入
		if (nCmdSize)
		{
			xData = m_vInspData.at(0);//取出vector第一瑕疵項目
			m_vInspData.erase(m_vInspData.begin());//清除第一瑕疵項目
		...
		if (pData)
		{
			...
			if (GetUANodeId(FIELD_Z_QXFLAG, xNode))//找的到缺陷刷新标志index的node
			{
				bool bValue = true;
				if (WriteBOOLField(xNode, bValue) == UA_STATUSCODE_GOOD)//寫值
				{
...
void CSystDONGGUANPPProcess::ProcessNewBatchDone()
{
	...
	if (!bFlag) 
	{
		...
		EnterCriticalSection(&m_xBatch);
#endif 
		int nCmdSize = m_vBatchData.size();//若工單資料有資料寫入
		if (nCmdSize)
		{
			xData = m_vBatchData.at(0);//取出vector第一工單項目
			m_vBatchData.erase(m_vBatchData.begin());//清除第一工單項目
		...
		if (pData) 
		{
			...
			UA_NodeId xNode;//mark flag to true
			if (GetUANodeId(FIELD_P_FLAG, xNode))//找的到缺陷刷新标志index的node
			{
				bool bValue = true;
				if (WriteBOOLField(xNode, bValue) == UA_STATUSCODE_GOOD)//寫值
				{
