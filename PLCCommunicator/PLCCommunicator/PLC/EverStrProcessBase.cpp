...
EverStrProcessBase* EverStrProcessBase::m_this = NULL;
///<summary>[Constructor]初始化</summary>
EverStrProcessBase::EverStrProcessBase()
{
	Init();
}
...
//===============================================================================
long EverStrProcessBase::ON_OPEN_PLC(LPARAM lp)
{
...
void EverStrProcessBase::ON_GPIO_NOTIFY(WPARAM wp, LPARAM lp)
{
	switch (wp)
	{
...
void EverStrProcessBase::Init()
{
	m_this = this;
	NotifyAOI(WM_SYST_PARAMINIT_CMD, NULL);
}
///<summary>終處置項目</summary>
void EverStrProcessBase::Finalize()
{
	for (int i = 0; i < TIMER_MAX; i++)
	{
...
void EverStrProcessBase::ProcessAOIResponse(LPARAM lp)
{
	switch (lp)//控制碼
	{
...
void EverStrProcessBase::ProcessResult()
{
...
void CALLBACK EverStrProcessBase::QueryTimer(HWND hwnd, UINT uMsg, UINT_PTR nEventId, DWORD dwTimer)
{
	if (m_this) {
		m_this->ProcessTimer(nEventId);
	}
}
void EverStrProcessBase::ProcessTimer(UINT_PTR nEventId)
{
	for (int i = 0; i < TIMER_MAX; i++) {
...
void EverStrProcessBase::SetInfoField(BATCH_SHARE_SYST_INFO& xInfo)
{
...
void EverStrProcessBase::ON_CCL_NEWBATCH()
{
...
void EverStrProcessBase::ON_C10_CHANGE(WORD wC10)
{
	CString strLog;
...
void EverStrProcessBase::PushResult(BATCH_SHARE_SYST_RESULTCCL& xResult)
{
	std::lock_guard< std::mutex > lock(m_oMutex);
	m_vResult.push_back(xResult);
}
DWORD EverStrProcessBase::Thread_Result(void* pvoid)
{
	EverStrProcessBase* pThis = (EverStrProcessBase*)pvoid;
	BOOL              bRun = TRUE;
	while (bRun)
	{
...
void EverStrProcessBase::WriteResult(BATCH_SHARE_SYST_RESULTCCL& xData)
{
#ifdef USE_IN_COMMUNICATOR
