...
{
	long lRtn = ERR_DLL_NOT_LOAD;
	PLC_DATA_ITEM_* pItem = GetPLCAddressInfo(nFieldId, FALSE);
	if (pItem){
		...
		if (pItem){
			...
			case PLC_VALUE_TYPE_::PLC_TYPE_STRING:
			{
				...
				for (int j = 0; j < nSize; j++){
					if (pItem->xValType == PLC_VALUE_TYPE_::PLC_TYPE_BIT && CString(pItem->cDevType) == L"D"){ //special case for D. ex:200.F
...
CString CPLCProcessBase::GET_PLC_FIELD_ADDRESS(int nFieldId)
{
	...
	if (nFieldId >= 0 && nFieldId < nFieldSize){
		const PLC_DATA_ITEM_ *pCur = GetPLCAddressInfo(nFieldId, FALSE);
		if (pCur){
			if (pCur->xValType == PLC_VALUE_TYPE_::PLC_TYPE_BIT && CString(pCur->cDevType) == L"D"){ //special case for D. ex:200.F
...
{
	if (m_hAOIWnd){
		::PostMessage(m_hAOIWnd, WM_GPIO_MSG, wp, lp);
	}
}
