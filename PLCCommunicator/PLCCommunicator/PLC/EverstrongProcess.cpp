...
EverstrongProcess* EverstrongProcess::m_this = NULL;
///<summary>[Constructor]初始化</summary>
EverstrongProcess::EverstrongProcess()
{
	Init();
}
...
long EverstrongProcess::ON_OPEN_PLC(LPARAM lp)
{
...
void EverstrongProcess::ON_GPIO_NOTIFY(WPARAM wp, LPARAM lp)
{
	switch (wp)
	{
...
void EverstrongProcess::Init()
{
	m_this = this;
	NotifyAOI(WM_SYST_PARAMINIT_CMD, NULL);
}
///<summary>終處置項目</summary>
void EverstrongProcess::Finalize()
{
	for (int i = 0; i < TIMER_MAX; i++)
	{
...
void EverstrongProcess::ProcessResult()
{
...
void CALLBACK EverstrongProcess::QueryTimer(HWND hwnd, UINT uMsg, UINT_PTR nEventId, DWORD dwTimer)
{
	if (m_this){
		m_this->ProcessTimer(nEventId);
	}
}
void EverstrongProcess::ProcessTimer(UINT_PTR nEventId)
{
	for (int i = 0; i < TIMER_MAX; i++){
...
void EverstrongProcess::ON_C10_CHANGE(WORD wC10)
{
	CString strLog;
...
void EverstrongProcess::PushResult(BATCH_SHARE_SYST_RESULTCCL &xResult)
{
	std::lock_guard< std::mutex > lock(m_oMutex);
	m_vResult.push_back(xResult);
}
DWORD EverstrongProcess::Thread_Result(void* pvoid)
{
	EverstrongProcess* pThis = (EverstrongProcess*)pvoid;
	BOOL              bRun = TRUE;
	while (bRun)
	{
...
void EverstrongProcess::WriteResult(BATCH_SHARE_SYST_RESULTCCL &xData)
{
#ifdef USE_IN_COMMUNICATOR
